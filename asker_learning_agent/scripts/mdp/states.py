...
class Action:
    def __init__(self, location=None, ask=False):
        self.ask = ask
        self.location = location
...
def state_action_reducer(state, action):
    ...
    vec_input = np.array([0.0 for x in range(8 + num_actions + 1)], np.float64)
    ...
    vec_input[8 + num_actions] = 1.0 if action.ask else 0.0    # 1 if this action involves asking a question, 0 otherwise.
...
def state_action_reduced_range():
    ...
    # Range for time
    ranges += [(0, 86400)]
     # Actions (w/ and w/o ask) and day of week take on values from 0 to 1
    ranges += [(0.0, 1.0) for x in range(num_actions + days_in_week + 1)]
